...
def classification_system(system_id):
    """Add or update or delete a single class of a classificationn system."""
    if request.method == "POST":
        ...
        try:
            classift_system = insert_classification_systems(classification_system_info)
        ...
        if 'classes' in request.files:
            ...
            try:
                insert_classes(post_file, classift_system.id)
            except Exception as e:
                db.session.rollback()
                abort(400, 'Error creating Classification System')
        ...
        return ClassificationSystemSchema(exclude=['id']).dump(classift_system), 201
    ...
    if request.method == "PUT":
        try:
            if 'classes' in request.files:
                ...
                json_classes_file = json.loads(classes_file)

            classification_system_info = request.form.to_dict(flat=True)

            class_system = update_classification_system(system_id, json_classes_file, **classification_system_info)
...
def get_class_system_classes(system_id):
...
def get_class_system_class(system_id, class_id):
...
@current_app.route("/classification_system/<system_id>/classes", defaults={'class_id': None}, methods=["POST"])
@current_app.route("/classification_system/<system_id>/classes/<class_id>", methods=["PUT", "DELETE"])
def class_system_class(system_id, class_id):
    ...
    if request.method == "POST":
        ...
        try:
            insert_class(class_system.id, class_id, **request.json)
        except Exception as e:
    ...
    if request.method == "PUT":
        ...
        try:
            update_class(class_system.id, class_id, **request.json)(class_system.id, class_id, **request.json)
        except Exception as e:
            abort(400, f'Error while update {class_id} class!')

    if request.method == "DELETE":
        try:
            delete_classes(system_id, class_id)
        except Exception as e:
